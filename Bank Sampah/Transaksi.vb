...
Public Class Transaksi
    ...
    Sub FakturOtomatis()
        Call Koneksi()
        myCommand = New MySqlCommand("select no_transaksi from setor_nasabah order by no_transaksi DESC", conn)
    ...
    Sub IsiNama()
        Try
            Call Koneksi()
            Dim sql As String
            sql = "select * from nasabah where id_nasabah = '" & TextId.Text & "'"
            myCommand = New MySqlCommand(sql, conn)
            myReader = myCommand.ExecuteReader()
            myReader.Read()
            If myReader.HasRows Then
                TextNama.Text = myReader.Item("nama_nasabah")
            End If
    ...
    Private Sub Button1_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button1.Click
        ...
        setoran = "insert into setor_nasabah values ('" & LblFaktur.Text & "','" & LblTanggal.Text & "','" & TextId.Text & "','A001','" & LabelJumlah.Text & "','" & LabelTotal.Text & "')"
